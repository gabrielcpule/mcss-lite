...
test('component tokens are re-declared in every theme block', () => {
  const css = files.get('src/tokens.css');
  assert.equal(css.match(/--button-primary-background:/g).length, 3);
});
...
test('Figma files are importable: no composites, no $root, px dimensions, aliases kept', () => {
  ...
  assert.equal(dark.color.action.primary.default.$value, '{color.blue.300}');
...
test('manifest lists every block and token', () => {
  ...
  assert.ok(m.tokens.some((t) => t.name === '--color-text-default' && t.value.dark === '#f8f9fa'));
